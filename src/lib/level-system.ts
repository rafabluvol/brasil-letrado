...
export interface LevelInfo {
  ...
  corBadge: string;
  descricao: string;
  /** Guarás needed to unlock this level */
  guarasNeeded: number;
}
...
export function calculateLevelFromXp(xp: number): number {
  for (let i = LEVELS.length - 1; i >= 0; i--) {
    if (xp >= LEVELS[i].guarasNeeded) return LEVELS[i].nivel;
  }
  return 1;
}
...
export function getXpForNextLevel(nivel: number): number {
  ...
  return next ? next.guarasNeeded : LEVELS[LEVELS.length - 1].guarasNeeded;
}
...
export function getUnlockedTrophies(guaras: number): Trophy[] {
  return TROPHIES.filter(t => guaras >= t.guarasRequired);
}

export function getNextTrophy(guaras: number): Trophy | null {
  return TROPHIES.find(t => guaras < t.guarasRequired) || null;
}

export function checkNewTrophies(oldXp: number, newXp: number): Trophy[] {
  return TROPHIES.filter(t => oldXp < t.guarasRequired && newXp >= t.guarasRequired);
}
